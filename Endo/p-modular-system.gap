...
# Arguments: 
# 	<module> is a MTX-module of <group>. 
# 	<p_prime_element> is a p'-element in <group>
# Return : 
...
## Debug >>>>>>>>>>>>>>>>>>>>>> 
	...
	# module := IrreducibleModules(group, field)[2][1];
	...
	# while true do 
	# 	p_prime_element := Random(group);
	# 	if Order(p_prime_element) mod prime <> 0 then break; fi;
	# od;
	# br := BrauerCharacterOfGModule(group, module, quotient_field, p_prime_element);

	# Print("p_prime_element : ", p_prime_element, "\n");
	# Print("Braucr : ", br, "\n");
## end Debug <<<<<<<<<<<<<<<<<<< 
